...
export default function ClaimDetailPage() {
  ...
  const loadClaim = useCallback(async () => {
    if (!token) return;
    try {
      const res = (await claimsApi.show(token, claimId)) as
        | { claim: ClaimDetail; voting: Record<string, unknown> }
        | ClaimDetail;
      // API returns { claim, voting } -- extract the claim object
      if ("claim" in res && res.claim) {
        const merged = {
          ...res.claim,
          ...(res.voting as Record<string, unknown>),
  ...
  const handleVote = async (e: React.FormEvent) => {
    ...
    setVoting(true);
    try {
      await claimsApi.vote(token, claim.id, {
    ...
    try {
      ...
      showToast(
        reviewDecision === "approve"
          ? "Claim approved. Voting emails sent to members."
          : "Claim rejected.",
      );
      await loadClaim();
    } catch (err: unknown) {
  ...
  if (!claim) {
    return (
      <EmptyState
        ...
        subtitle="This claim may have been removed or you don't have access."
      />
    );
  }
  ...
  return (
    <div className="main-container-wrapper claim-detail-wrapper">
      ...
      <div className="flex mb-5">
        <div className="mr-6 claim-right-asside">
          {/* Claimant Info */}
            ...
            {claim.claimant_image || claim.claimant?.profile_picture ? (
                ...
                <img
                  src={(() => {
                    const pic =
                      ...
                      claim.claimant?.profile_picture ||
                      "";
              ...
              <div
                ...
                style={{
                  ...
                  fontSize: 20,
                  fontWeight: 600,
                  color: "var(--sidebar-nemu-active-color)",
                }}
              >
                {(claim.claimant_name || claim.claimant?.first_name || "?")
                  .charAt(0)
                  .toUpperCase()}
              </div>
            )}
                ...
                <div className="vote-result">
                  ...
                  {claim.can_vote && !claim.has_voted ? (
                    <form onSubmit={handleVote} className="mt-3">
                      {voteError && (
                        <div className="login-error mb-3">{voteError}</div>
                      )}
                    ...
                    <h6 className="mt-3">
                      You have already participated in the voting process.
                    </h6>
                  ) : (
                    <h6 className="mt-3">
                      You are unable to participate in the voting process.
                    </h6>
                  )}
                  <div
                  ...
                  >
                    Voting deadline:{" "}
                    {new Date(claim.voting_deadline).toLocaleString()}
                  </div>
                </div>
              ) : (
                <div className="vote-result">
        ...
        {(claim.total_participants ?? 0) > 0 && (
          <VotingResultChart
            totalParticipants={
              claim.vote_stats?.total ?? claim.total_participants ?? 0
            }
            approvedVotes={
              claim.vote_stats?.approved ?? claim.approved_votes ?? 0
            }
        ...
        <div className="voting-content-wrapper">
          <div className="vote-result">
            ...
            <p
              ...
              style={{ color: "var(--text-default-color)" }}
            >
              As the group owner, please review this claim and decide whether to
              approve it for member voting or reject it.
            </p>
            <form onSubmit={handleReview} className="mt-3">
              {reviewError && (
                <div className="login-error mb-3">{reviewError}</div>
              )}
              <div className="form-group">
                <label>Your Decision</label>
                <div className="flex gap-3 mt-1">
      ...
      {claim.status === "rejected" && claim.owner_review_reason && (
        <div className="voting-content-wrapper">
          <div className="vote-result">
            ...
            <p className="mt-2" style={{ color: "#e74c3c" }}>
              {claim.owner_review_reason}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="voting-content-wrapper">
          <div className="vote-result">
            ...
            <p
              ...
              style={{ color: "var(--text-default-color)" }}
            >
              This claim is pending review by the group owner. Voting will begin
      ...
      <div className="groups-data-content-wrapper">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
          <div>
            ...
            <div className="data-content">
              {claim.date_of_incident?.split("T")[0]}
            </div>
          </div>
          <div>
            <div className="data-title">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
          ...
          <div>
            ...
            <div className="data-content">
              {claim.estimated_cost_of_repairs}
            </div>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
          ...
          <div>
            <div className="data-title">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>{" "}
              Damage Description
            </div>
          ...
          <div>
            <div className="data-title">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="2" /><path d="M16 8h3l3 3v5h-6V8z" /><circle cx="5.5" cy="18.5" r="2.5" /><circle cx="18.5" cy="18.5" r="2.5" /></svg>{" "}
              Vehicle Details
            </div>
        ...
        {/* Evidence */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 claim-doc-container">
          <div>
            <div className="data-title">
              <svg
                width="14"
            ...
            <div className="data-content">
              {claim.photos_url && (
                <a
                  href={claim.photos_url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <img
                    src={claim.photos_url}
                    width={80}
                    alt="damage"
          ...
          <div>
            ...
            <div className="data-content">
              {(claim.police_report_url || claim.report_url) && (
                <a
                  href={claim.police_report_url || claim.report_url || ""}
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          <div>
            ...
            <div className="data-content">
              {claim.signature_url && (
                <a
                  href={claim.signature_url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <img
                    src={claim.signature_url}
                    width={80}
                    alt="signature"
                    style={{ borderRadius: 4 }}
                  />
...
function VotingResultChart({
  ...
  const approvedPct =
    eligibleVoters > 0 ? (approvedVotes / eligibleVoters) * 100 : 0;
  const deniedPct =
    eligibleVoters > 0 ? (deniedVotes / eligibleVoters) * 100 : 0;
  const notVotedPct =
    eligibleVoters > 0 ? (notVoted / eligibleVoters) * 100 : 0;
      ...
      <div className="vote-result">
        ...
        <div className="vr-chart-wrapper">
            ...
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
              ...
              {eligibleVoters > 0 &&
                (() => {
                  ...
                  const x1 =
                    size / 2 + (radius - strokeWidth / 2 - 2) * Math.cos(rad);
                  const y1 =
                    size / 2 + (radius - strokeWidth / 2 - 2) * Math.sin(rad);
                  const x2 =
                    size / 2 + (radius + strokeWidth / 2 + 2) * Math.cos(rad);
                  const y2 =
                    size / 2 + (radius + strokeWidth / 2 + 2) * Math.sin(rad);
          ...
          <div className="vr-stats">
            ...
            <div className="vr-stat-row">
              ...
              <span className="vr-stat-value">
                {totalParticipants} / {eligibleVoters}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="vr-threshold">
          <div className="vr-threshold-header">
            ...
            <span
              className={`vr-threshold-badge ${thresholdMet ? "vr-badge-met" : "vr-badge-not-met"}`}
            >
              {status === "approved"
                ? "Claim Approved"
                : thresholdMet
          ...
          <div className="vr-threshold-bar">
            <div
              className="vr-threshold-fill"
              style={{ width: `${Math.min(approvalPercentage, 100)}%` }}
            />
            <div
              className="vr-threshold-marker"
